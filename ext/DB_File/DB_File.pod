...
B<DB_File> is a module which allows I<Perl5> programs to make use of 
...
Berkeley DB is a C library which provides a consistant interface to a number of 
database formats. 
...
other hashing packages like I<dbm>, I<ndbm>, I<odbm>, I<gdbm> and I<sdbm>.
Remember though, the files created using DB_HASH are 
not compatable with any of the other packages mentioned.
...
to perform the comparision of keys. By default, though, the keys are stored 
in lexical order.
...
B<DB_File> allows access to Berkeley DB files using the I<tie> mechanism 
in Perl5 (for
full details of I<tie> refer to the main perl manual pages).
This facility allows B<DB_File> to access Berkeley DB files using either an
associative array (for DB_HASH & DB_BTREE file types) or an ordinary 
array (for the DB_RECNO file type).

In addition to the I<tie> interface, it is also possible to use most of the
functions provided in the Berkeley DB API.


=head2 Differences with Berkeley DB

Berkeley DB uses the function I<dbopen> to open or create a 
database. Below is the C prototype for I<dbopen>.

      DB*
      dbopen (const char * file, int flags, int mode, 
              DBTYPE type, const void * openinfo)

The parameter I<type> is an enumeration which specifies which of the 3
interface methods (DB_HASH, DB_BTREE or DB_RECNO) is to be used. 
Depending on which of these is actually
chosen, the final parameter, I<openinfo> points to a data structure which
allows tailoring of the specific interface method. 
...
The I<filename>, I<flags> and I<mode> parameters are the direct equivalent 
of their I<dbopen> counterparts. The final parameter I<$DB_HASH>
performs the functon of both the I<type> and I<openinfo>
parameters in I<dbopen>.

In the example above I<$DB_HASH> is actually a reference to a hash object.
...
So, for example, the I<$DB_HASH> reference will only allow keys called I<bsize>,
I<cachesize>, I<ffactor>, I<hash>, I<lorder> and I<nelem>. 
...
Berkeley DB allows the creation of in-memory databases by using NULL in 
place of the filename. 
B<DB_File> uses I<undef> instead of NULL to provide this functionality.
...
All the functions defined in the I<dbopen> manual page are available except
for I<close> and I<dbopen> itself.  
...
All the constants defined in the I<dbopen> manaul page are also available.
...
Same as in I<recno> except that the flags parameter is optional. 
Remember the value
associated with the key you request is returned in the I<$value> parameter.
...
the default comparision function will not be used. Instead a Perl sub, I<Compare>,
...
I<perl>(1), I<dbopen>(3), I<hash>(3), I<recno>(3), I<btree>(3) 

Berkeley DB is available from cs.berkeley.edu in the directory /ucb/4bsd.
